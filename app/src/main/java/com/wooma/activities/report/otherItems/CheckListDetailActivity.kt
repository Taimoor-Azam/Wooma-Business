...
class CheckListDetailActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        questionAdapter = InventoryCheckListQuestionAdapter(
            ...
            onAnswerSelected = { question, answerOption ->
                val index =
                    checkListQuestionItems.indexOfFirst { it.checklist_question_id == question.checklist_question_id }
                if (index != -1) {
                    checkListQuestionItems[index].answer_option = answerOption
                    upsertQuestionAnswerApi(
                        checkListQuestionItems[index],
                        answerOption,
                        question.note,
                        false
                    )
    ...
    private fun uploadQuestionPhotos(questionId: String, uris: List<Uri>) {
        makeApiRequest(
            ...
            listener = object :
                ApiResponseListener<ApiResponse<ChecklistAnswerAttachmentResponse>> {
