...
interface PendingUploadDao {
    ...
    @Query(
        "SELECT COUNT(*) FROM pending_uploads pu WHERE pu.status = 'PENDING' AND (" +
            "(pu.entityType = 'REPORT_COVER' AND pu.entityLocalId = :reportId) OR " +
            "(pu.entityType = 'METER' AND EXISTS (" +
                "SELECT 1 FROM meters m WHERE m.id = pu.entityLocalId AND m.reportId = :reportId" +
            ")) OR " +
            "(pu.entityType = 'KEY' AND EXISTS (" +
                "SELECT 1 FROM `keys` k WHERE k.id = pu.entityLocalId AND k.reportId = :reportId" +
            ")) OR " +
            "(pu.entityType = 'DETECTOR' AND EXISTS (" +
                "SELECT 1 FROM detectors d WHERE d.id = pu.entityLocalId AND d.reportId = :reportId" +
            ")) OR " +
            "(pu.entityType = 'ROOM_ITEM' AND EXISTS (" +
                "SELECT 1 FROM room_items ri INNER JOIN rooms r ON r.id = ri.roomId " +
                "WHERE ri.id = pu.entityLocalId AND r.reportId = :reportId" +
            ")) OR " +
            "(pu.entityType = 'ROOM_INSPECTION' AND EXISTS (" +
                "SELECT 1 FROM room_inspections ri INNER JOIN rooms r ON r.id = ri.roomId " +
                "WHERE ri.id = pu.entityLocalId AND r.reportId = :reportId" +
            ")) OR " +
            "(pu.entityType = 'CHECKLIST_ANSWER_ATTACHMENT' AND EXISTS (" +
